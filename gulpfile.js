...
const copy = () =>
  gulp
    .src(['src/resources/**/*.*', 'src/resources/**/.*', '!src/resources/**/.keep'], {
      base: 'src/resources',
      dot: true
    })
...
const lintPug = () => gulp.src(['src/*.pug', 'src/pug/**/*.pug']).pipe(plumber({ errorHandler }));
...
const validateHtml = () => gulp.src('build/**/*.html').pipe(plumber({ errorHandler }));
...
const watch = () => {
  ...
  gulp.watch(['src/images/sprites/png/*.png', 'src/scss/_sprites.hbs'], spritesPng);
